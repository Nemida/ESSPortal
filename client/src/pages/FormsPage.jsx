...
const FormsPage = () => {
  ...
  const filteredForms = useMemo(() => {
    return allForms.filter(form => {
      ...
      const searchMatch = !searchTerm || (form.search_tags && form.search_tags.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="bg-white p-8 rounded-lg shadow-lg">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          ...
          <FilterButton category="Leave Management" />
          <FilterButton category="IT Services" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredForms.length > 0 ? (
            filteredForms.map((form) => {
              ...
              const badgeClass = badgeColorMap[form.badge_color] || 'bg-gray-500';
              ...
              <Link to={`/forms/${form.form_id}`} key={form.form_id} className="block bg-white rounded-lg shadow border overflow-hidden transform transition-transform hover:-translate-y-1 hover:shadow-xl">
                ...
                <div className="px-5 py-3 bg-gray-50 flex justify-between items-center">
                   <div className="text-xs text-gray-500">
                     <span>Updated: {form.last_updated ? new Date(form.last_updated).toLocaleDateString() : 'N/A'}</span> | <span>{form.version || 'N/A'}</span>
                   </div>
